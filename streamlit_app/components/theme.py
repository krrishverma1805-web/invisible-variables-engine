...
.carbon-tag--purple { background: #f6f2ff; color: #6929c4; }
...
@media (prefers-color-scheme: dark) {
    ...
    .carbon-tag--purple { background: #1c0f30; color: #be95ff; }
}
</style>
"""
...
def carbon_tag(text: str, color: str = "blue") -> str:
    ...
    Args:
        ...
        color: One of ``blue``, ``red``, ``green``, ``yellow``, ``gray``, ``purple``.
    """
...
def explanation_source_badge(
    source: str,
    *,
    pending: bool = False,
    status: str | None = None,
) -> str:
    """Return the badge HTML for an explanation's provenance.

    Per plan §A1 + §107: surface the LLM model used + a tooltip-style title
    so power users can verify the prose is AI-generated and that all cited
    statistics are validated.

    Args:
        source: One of ``"llm"`` or ``"rule_based"``.
        pending: True when the LLM enrichment is still queued — show a
            "generating…" badge so the UI is honest about the wait.
        status: Optional underlying ``llm_explanation_status`` string used
            to render distinct ``failed`` and ``disabled`` badges.

    Returns:
        Empty string when no badge is appropriate (e.g. plain rule-based
        in a non-pending state and not failed/disabled).
    """
    from html import escape

    if pending:
        return (
            '<span class="carbon-tag carbon-tag--yellow" '
            'title="An AI-assisted version is being generated. Refresh shortly.">'
            "AI generating…</span>"
        )

    if source == "llm":
        return (
            '<span class="carbon-tag carbon-tag--purple" '
            'title="Generated by an AI model with all cited statistics validated against the pipeline output.">'
            "AI-assisted</span>"
        )

    safe_status = escape(status) if status else ""
    if safe_status == "failed":
        return (
            '<span class="carbon-tag carbon-tag--red" '
            'title="AI generation failed validation; rule-based prose is shown instead.">'
            "Rule-based · AI failed</span>"
        )
    if safe_status == "disabled":
        return (
            '<span class="carbon-tag carbon-tag--gray" '
            'title="AI explanations are disabled — either by deployment policy or because this finding references a non-public column.">'
            "Rule-based · AI off</span>"
        )
    return ""  # No badge for plain rule-based in normal flow.
